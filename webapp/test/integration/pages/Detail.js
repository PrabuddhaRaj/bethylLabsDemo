		...
		onTheDetailPage : {
			...
			assertions : {
				...
				theLineItemsListShouldHaveTheCorrectNumberOfItems : function () {
					return this.waitFor(this.createAWaitForAnEntitySet({
						entitySet : "to_PurchaseContractItem",
						success : function (aEntitySet) {
							...
							return this.waitFor({
								...
								check: function (oList) {
									...
									var sObjectID = oList.getBindingContext().getProperty("PurchaseContract");

									var iLength = aEntitySet.filter(function (oLineItem) {
										return oLineItem.PurchaseContract === sObjectID;
									}).length;
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							return this.waitFor({
								id : "lineItemsTitle",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name: "text", value: "<to_PurchaseContractItemPlural> (" + iNumberOfItems + ")"}),
								success : function () {
