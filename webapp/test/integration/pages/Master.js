		...
		onTheMasterPage : {
			...
			actions : {
				...
				iSortTheListOnName : function () {
					return this.iChooseASorter("sortButton", "Sort By <PurchaseContract>");
				},
				iSortTheListOnUnitNumber : function () {
					return this.iChooseASorter("sortButton", "Sort By <PurchaseContractTargetAmount>");
				},

				iFilterTheListOnUnitNumber : function () {
					return this.iMakeASelection("filterButton", "<PurchaseContractTargetAmount>", "<100 <DocumentCurrency>");
				},

				iGroupTheList : function () {
					return this.iChooseASorter("groupButton", "<PurchaseContractTargetAmount> Group");
				},
						...
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("PurchaseContract") === oObject.PurchaseContract;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].PurchaseContract;
									} else {
										sCurrentId = aItemsNotInTheList[0].PurchaseContract;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("A_PurchaseContract", {
										PurchaseContract : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("PurchaseContract"),
						title: oBindingContext.getProperty("PurchaseContract")
					};
				}
			},
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						...
						id: "masterPageTitle",
						autoWait: false,
						matchers: new PropertyStrictEquals({name : "text", value : "<A_PurchaseContract> (0)"}),
						success: function () {
				...
				theListShouldBeFilteredOnUnitNumber : function () {
					return this.theListShouldBeFilteredOnFieldUsingComparator("PurchaseContractTargetAmount", 100);
				},


				theListShouldBeSortedAscendingOnUnitNumber : function () {
					return this.theListShouldBeSortedAscendingOnField("PurchaseContractTargetAmount");
				},

				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("PurchaseContract");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all A_PurchaseContract to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "masterPageTitle",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "text", value : "<A_PurchaseContract> (" + iExpectedLength + ")"}),
								success : function () {
